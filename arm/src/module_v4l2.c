...
static int do_v4l2InputGetFormat(V4L2Input* _v4l2, ImageDescription* _imageDesc) {
...
static int do_v4l2InputMmapBuffers(V4L2Input* _v4l2) {
  ...
  for (/*bufferIndex*/; bufferIndex < sizeof(_v4l2->m_buffers) / sizeof(*_v4l2->m_buffers); ++bufferIndex) { // fill unused buffers
...
static int do_v4l2InputPutFrame(V4L2Input* _v4l2, size_t _frameIndex) {
  ...
  if (_frameIndex >= sizeof(_v4l2->m_buffers) / sizeof(*_v4l2->m_buffers) || _v4l2->m_buffers[_frameIndex] == MAP_FAILED)
    return ECHRNG;
